...
│
├── skill/                    ← Knowledge base the agent reads as it works
...
│       └── basilica_gpu_training.md  ← Optional: train on decentralized GPUs via Basilica (SN39)
│
└── external_skills/          ← Created at runtime — agent fetches these when needed
    └── (downloaded on demand per External Skills table below)
```
...
- Optional: Basilica (SN39) integration for training on decentralized GPUs
...
elsewhere. The agent prompt tells Ralph when and how to fetch these. They get installed
into `external_skills/` at runtime.

| Skill | What It Does | When Needed | Source |
|-------|-------------|-------------|--------|
| **basilica-cli-helper** | Basilica CLI commands, GPU instance management, SSH troubleshooting | Training on Basilica GPUs | [mcpmarket.com](https://mcpmarket.com/tools/skills/basilica-cli-helper) |
| **bittensor-subnet-design** | General Bittensor subnet design patterns and CLI reference | Understanding SN50 incentive mechanism | Pre-installed in Claude |

### Adding Your Own

To plug in another skill (e.g., a PyTorch profiler, a monitoring stack, a different data source):

1. Add it to the table above in this README
2. Add a row to the "Available External Skills" table in `AGENT_PROMPT.md`
3. Reference it in the relevant phase with instructions on when to read its SKILL.md

The agent treats `external_skills/` as a plugin directory — anything in there with a
`SKILL.md` gets read when the relevant phase triggers it.
